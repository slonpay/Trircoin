...
#if defined(NDEBUG)
# error "Trircoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x1761056ba2629fffc91fdd111be733b758b01af42a4e131b90e7e0e8bdbe81c8");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Trircoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Trircoin Signed Message:\n";
...
{
    // Trircoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // Trircoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Trircoin: 1 days
static const int64 nTargetSpacing = 1 * 60; // Trircoin: 1 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Trircoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Trircoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x18a71df186553bdadba5b2d2bd2ae97a7270c17d2b2e679a8c053de3c0acfc44");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "El Peruano: Congreso evalua hoy pedido para remover a los miembros del CNM";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 20 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0402022862fbed719fff117bdd86a9aae04023efc4b842b7f370f8e446f709a59c2fa6c6a7bad58e3bf6209639ae6ff043f767b15fd89ca1773a05778a94debdb6") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1531237263;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085491818;

        if (fTestNet)
        {
            block.nTime    = 1531237238;
            block.nNonce   = 386914224;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x706cc552217045ededd459214c0e3adbf62eba145b5d59871c9bd10ccd8da275"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc3, 0xba, 0xd3 }; // Trircoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("TrircoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("TrircoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static TrircoinMiner(CWallet *pwallet)
{
    printf("TrircoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("trircoin-miner");
    ...
    try { loop {
        ...
        printf("Running TrircoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("TrircoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&TrircoinMiner, pwallet));
}
